//diff between let , var , const
let amount = 600;
//scope difference: 
//var:function level scope and let,const :block scope
function learningLetVarConstDifference() {
    //code block 
    var a = 10 ;
    let b = 40

    if(true){
    let c = 50; //can not access  variable with let keyword outside block
                //let, keyword has block level scop
    var d = 90; //var keyword has function level scop
    const e = 80;
    // let res = a + 20;
    //console.log(res);
    //console.log(b);
    console.log(c); //50
    console.log(d); //90
    console.log(e); //80
  }
  
  console.log(d);  
  //console.log(c); //uncought ReferenceError: error
  //console.log(c); //uncought ReferenceError: error
  

}

//learningLetVarConstDifference();

//hosting : define value to the variable first and then declare it
function test(){

    //let name = 'poonam'; //D+D
    let name;  //declaration
    name = 'poonam';  //defination

    surname = 'patil'; //def
 //   let surname; //declr with let hoisting is not possible 
 //   console.log(name,surname);

     city = 'mumbai';
     var city;
     console.log(city);  //hoisting is possible with var keyword

     city2 = 'pune';
     //const city2; //with const keyword hoisting is not possible
}

test();

reAssigment();

function reAssigment(){
  let name = 'ruks'; //Reassigning value to the variable is possible with let keyyword
  name = 'vinita';
  name = 'jdjdjd';
  name : 'kksks'
  console.log(name);

  var surName = 'patil';
  surName = 'jagdale';
  surName = 'kxkxkxk'
  console.log(surName); //Reassigning value to the variable is possible with var keyyword

  const middleName = "Om";
  // middleName = 'Jay'; //
    // console.log(middleName);
}

//Redeclaration of varibles

reDeclaration();

function reDeclaration(){
  let city = 'pune';  //Redeclaration is not possible with variable having let keyword.
  //let city ='mopj';
  let amount = 888
  amount = 4444;
  console.log(amount);
  var num = 9000;
  var num = 8000000; //Redeclaration of variable having var keyword is possible .
  console.log(num);

  const hhh = true;  //Redeclaration of variable having const keyword is not possible
  //const hhh = false;

}

function amountCal(){
  console.log(amount);
}
amountCal()

